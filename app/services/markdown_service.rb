class MarkdownService
  ...
  def call
    ...
    rendered_html, mention_participant_ids = mentions_to_links(rendered_html: rendered_html, mentions_cache: @mentions_cache) if @mentions_cache.present?
    [rendered_html, mention_participant_ids]
  end
  ...
  def mentions_to_links(rendered_html:, mentions_cache:)
    ...
    mentions_cache.each do |mention|
      rendered_html.gsub! /@(#{mention['name']})/ do |match|
        ...
        "<a href='/participants/#{mention['id']}' target='_blank'>@#{Regexp.last_match(1)}</a>"
      end
    end
    [rendered_html, mention_participant_ids]
  end
